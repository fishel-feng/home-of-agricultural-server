...
public class CommonCacheUtil {
        ...
        if (pool != null) {
            try (Jedis jedis = pool.getResource()) {
                ...
                try {
                    ...
                    trans.sadd(USER_PREFIX + userElement.getTel(), userElement.getToken());
                    trans.exec();
                } catch (Exception e) {
                    trans.discard();
    ...
    public int cacheForVerificationCode(String key, String verCode, String type, int second, String ip) throws MyServerException {
        JedisPool pool = jedisPoolWrapper.getJedisPool();
        if (pool != null) {
            try (Jedis jedis = pool.getResource()) {
                jedis.select(0);
                ...
                throw new MyServerException("Fail to cache for expiry");
            }
        }
        return 0;
    }
